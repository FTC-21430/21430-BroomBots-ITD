...
@TeleOp
public class DemoTeleop_OneDriver extends BaseTeleOp {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            // toggles manual mode
            if (gamepad1.options && !gp1shareold){
                if (manualMode){
                    manualMode = false;
                }else{
                    manualMode = true;
...
//          claw logic
            ...
            if (robot.spampleArm.currentArmState != SpampleArm.armState.calibrating) {
                ...
                if (!grabbingSample || !lookingForSample) {
                    ...
                    if (gamepad1.dpad_down && !grabbingSample) {
                    ...
                    if (robot.spampleArm.getTwist() <= 90 && robot.spampleArm.getTwist() >= -90) {
                        double twisting_input = 0;
                        if (gamepad1.dpad_left){
                            twisting_input = -1;
                        }else if (gamepad1.dpad_right){
                            twisting_input = 1;
                        }
                        robot.spampleArm.rotateTwistTo(robot.spampleArm.getTwist() +  twisting_input * robot.getDeltaTime() * 180);
